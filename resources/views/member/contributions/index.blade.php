        ...
        <div class="bg-white shadow-sm rounded-lg overflow-hidden">
            @if ($contributions->isEmpty())
                <div class="p-8 text-center">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('member.contributions.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            Make a Contribution
                        </a>
                    </div>
                </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $index + 1 + ($contributions->currentPage() - 1) * $contributions->perPage() }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                    {{ number_format($contribution->amount) }} TZS
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    @if($contribution->status === 'paid')
                                        <a href="{{ route('member.contributions.show', $contribution->id) }}" class="text-indigo-600 hover:text-indigo-900 flex items-center">
