...
<div class="container mx-auto py-8 max-w-lg">
    ...
    <form method="POST" action="{{ route('super_admin.members.update', $member) }}">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label class="block mb-1">Name</label>
            <input type="text" name="name" class="form-input w-full" value="{{ old('name', $member->user->name) }}" required>
            @error('name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1">Email</label>
            <input type="email" name="email" class="form-input w-full" value="{{ old('email', $member->user->email) }}" required>
            @error('email') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1">Phone</label>
            <input type="text" name="phone" class="form-input w-full" value="{{ old('phone', $member->user->phone) }}" required>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="dob" class="form-input w-full" value="{{ old('dob', $member->dob) }}">
